...
Data <- read_excel("/Users/yangwuyue/Downloads/review/Data_log10.xlsx")
...
fit <- sem(model, data = covid, se = 'bootstrap', bootstrap = 5000, optim.method = "BFGS", check.gradient = FALSE, verbose = TRUE)
resull <- summary(fit, standardized = T, rsquare = T)
results<-parameterEstimates(fit)

write.csv(results, file = "results_Spreading_rate_0814.csv", row.names = FALSE)
save(fit, file = "fit_Spreading_rate.RData")
load("fit_Spreading_rate.RData")
##plot
library(semPlot) 
# Incomesupport + Testingpolicy + Contacttracing
# Suspectedcase + Detectionandtreatment

png("Spreading_rate_0814.png", width=20, height=20, units="in", res=300)
par(family="Arial")
