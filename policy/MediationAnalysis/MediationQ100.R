...
Data <- read_excel("/Users/yangwuyue/Downloads/review/data0812.xlsx")
...
colnames(covid)[colnames(covid) == "Half_time"] <- "Y6"


model <- '
  Z1 =~ x1+x2+x3+x4+x5+x6+x7+x8+x9+x10
  Z2 =~ x11 + x12 + x13
  Z3 =~ x14 + x15
  Z4 =~ x16
  ...
  # ????????
  Y6 ~ m1*M1 + m2*M2 + m3*M3 + m4*M4 + m5*M5 + c1*Z1 + c2*Z2 + c3*Z3 + c4*Z4
...
fit <- sem(model, data = covid, bootstrap = 5000, optim.method = "BFGS",  verbose = TRUE)
...
write.csv(results, file = "results_Half_time.csv", row.names = FALSE)
##plot
library(semPlot) 
# Incomesupport + Testingpolicy + Contacttracing
# Suspectedcase + Detectionandtreatment
emTools::semTools("device", width = 10, height = 10, res = 300)  
semPaths(fit, what = "Estimate", layout = list(nodeLabels = list(fontface = "bold"), 
                                             edgeLabels = list(fontface = "bold"))) 
